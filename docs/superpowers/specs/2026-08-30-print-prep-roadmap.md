...
| SP-4c | X-ray / clipping | — | SHIPPED, branch `worktree-sp4c-xray-clipping`, commits c3802ea..3a12a33. `src/store/viewerStore.ts` gains x-ray + clip-plane state (5 fields + setters), `src/components/Viewer3D.tsx` x-ray lifecycle + clip-plane lifecycle + interlock with repair and heatmaps + `renderer.localClippingEnabled`, `src/components/prepare/AnalysisSection.tsx` inspect group (x-ray toggle, clip toggle, axis/position/flip controls), `HelpModal.tsx` entry, `e2e/xray-clipping.spec.ts` e2e hard gate. Final review fix wave 3a12a33: `splitByShell` restores and disarms x-ray/clip before cloning part materials (was stranding parts translucent), `ExportDialog.runExport` disarms x-ray around collectExportMeshes then re-arms (GLB/glTF was baking the 0.32 opacity). Spec: `2026-09-06-sp4c-xray-clipping-design.md`; plan: `../plans/2026-09-06-sp4c-xray-clipping.md`. |
...
| SP-5b | Auto-orient | SP-5a | SHIPPED, branch `worktree-sp5b-auto-orient`, commits 63f306a..8597f42. `src/services/autoOrient.ts` (`computeBestOrientation`: candidate rest directions = de-duplicated face normals, largest coplanar area first, capped at 64, plus a 64-point Fibonacci sweep; cost = overhang-area fraction that is not bed-contact + 0.2 height + minus 0.15 bed contact; `contactEps` is diag-scaled so it is orientation independent; size gate at 200k faces), `src/components/Viewer3D.tsx` `autoOrient` handle (merge world geometry, premultiply the winning quaternion, drop to floor + centre on plate, one "Auto-orient" undo entry), `src/components/prepare/TransformSection.tsx` button + result note, `src/components/HelpModal.tsx` entry, `e2e/auto-orient.spec.ts`. Fix wave (final review): `contactEps` made diag-scaled (was candidate-height-scaled, which a sub-mm base nub could trip and flip the model); face-normal cap 256 -> 64; `Box3.expandByObject` switched to precise at the model-dimension / grid sites in `Viewer3D.tsx` (`updateGeometryDetails`, `rebuildGrid`, `refreshSceneEnvironment`, `getModelDimensions*`) so the reported box is tight after a rotation (this also corrects the shipped Rotate action); `TransformSection.test.tsx` `beforeEach` hoisted to file scope (the auto-orient tests were passing on leaked store state). Spec: `2026-09-07-sp5b-auto-orient-design.md`; plan: `../plans/2026-09-07-sp5b-auto-orient.md`. |
| SP-5c | Bed layout | SP-5a | SHIPPED, branch `worktree-sp5c-bed-layout`, commits a373ab3..01f2731. `src/services/bedLayout.ts` (`computeBedLayout`: shelf pack, sort depth then width descending, row wrap on `bed.x`, unplaced on too-wide or `bed.z` overflow, centre on origin), `src/components/Viewer3D.tsx` `arrangeOnPlate` handle (tight world footprint per model root, `buildVolumeMm / modelUnitInMm` bed, ~3 mm gap, one "Arrange on plate" undo entry keyed by root uuid; preview + multi-model roots, not split parts), `src/components/prepare/TransformSection.tsx` button + result note, `src/components/HelpModal.tsx` entry, `e2e/bed-layout.spec.ts`. Spec: `2026-09-07-sp5c-bed-layout-design.md`; plan: `../plans/2026-09-07-sp5c-bed-layout.md`. |
