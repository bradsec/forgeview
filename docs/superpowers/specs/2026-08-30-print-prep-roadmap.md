...
| # | Piece | Depends on | Status |
|---|-------|-----------|--------|
| SP-5a | Build-volume box | — | SHIPPED, branch `worktree-sp5a-build-volume-box`, commits 7bf4198..49857e8. `src/store/viewerStore.ts` `showBuildVolume` flag, `src/services/buildVolumeOverlay.ts` (wireframe box + footprint grid group, tagged `userData.buildVolumeOverlay`), `src/components/Viewer3D.tsx` Effect 23 lifecycle (no interlock), `src/services/exporters.ts` skip tag, `src/components/prepare/ScaleSection.tsx` Show build volume toggle, `src/components/HelpModal.tsx` entry, `e2e/build-volume-box.spec.ts`. Spec: `2026-09-07-sp5a-build-volume-box-design.md`; plan: `../plans/2026-09-07-sp5a-build-volume-box.md`. |
