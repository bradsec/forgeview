...
| SP-4c | X-ray / clipping | — | SHIPPED, branch `worktree-sp4c-xray-clipping`, commits c3802ea..47668a1. `src/store/viewerStore.ts` gains x-ray + clip-plane state (5 fields + setters), `src/components/Viewer3D.tsx` x-ray lifecycle + clip-plane lifecycle + interlock with repair and heatmaps + `renderer.localClippingEnabled`, `src/components/prepare/AnalysisSection.tsx` inspect group (x-ray toggle, clip toggle, axis/position/flip controls), `HelpModal.tsx` entry, `e2e/xray-clipping.spec.ts` e2e hard gate. Spec: `2026-09-06-sp4c-xray-clipping-design.md`; plan: `../plans/2026-09-06-sp4c-xray-clipping.md`. |
