...
| SP-4b | Wall-thickness heatmap | — | SHIPPED, branch `worktree-sp4b-wall-thickness`, commits 4df392e..aeabf15. `src/services/wallThickness.ts`, `wallThicknessOverlay.ts`, `three-mesh-bvh` 0.9.14 npm dep, `GeometryDetails` gains `thinWallFaceCount`, `prepChecks` wired for Thin walls readiness row, `exporters.ts` excludes overlay tag, `Viewer3D.tsx` computes live count and owns overlay lifecycle, `AnalysisSection.tsx` second tool, `HelpModal.tsx` entry. Spec: `2026-09-06-sp4b-wall-thickness-heatmap-design.md`; plan: `../plans/2026-09-06-sp4b-wall-thickness-heatmap.md`. |
