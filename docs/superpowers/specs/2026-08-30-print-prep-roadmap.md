...
| SP-5b | Auto-orient | SP-5a | SHIPPED, branch `worktree-sp5b-auto-orient`, commits 63f306a..6e6e771. `src/services/autoOrient.ts` (`computeBestOrientation`: candidate rest directions = de-duplicated face normals, largest coplanar area first, plus a 64-point Fibonacci sweep; cost = overhang-area fraction that is not bed-contact + 0.2 height + minus 0.15 bed contact; size gate at 200k faces), `src/components/Viewer3D.tsx` `autoOrient` handle (merge world geometry, premultiply the winning quaternion, drop to floor + centre on plate, one "Auto-orient" undo entry), `src/components/prepare/TransformSection.tsx` button + result note, `src/components/HelpModal.tsx` entry, `e2e/auto-orient.spec.ts`. Note: `Box3.expandByObject` with precise=false reports a loose box for a rotated model, so the Details readout dimensions are approximate after any rotation (pre-existing, shared with Rotate). Spec: `2026-09-07-sp5b-auto-orient-design.md`; plan: `../plans/2026-09-07-sp5b-auto-orient.md`. |
