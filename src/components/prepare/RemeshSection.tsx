...
export function RemeshSection({ onRemesh }: RemeshSectionProps) {
  ...
  const run = async () => {
    ...
    try {
      const result = await onRemesh(operation === 'decimate' ? { operation, targetTriangles: target } : { operation, resolution }, active.signal)
  ...
  const valid = operation === 'decimate' ? Number.isInteger(target) && target >= 4 : Number.isInteger(resolution) && resolution >= 8 && resolution <= 64
  return <div>
    ...
    {operation === 'decimate' ? <>
      <label className="mt-2 block text-xs">Target triangles
        <input className="ml-2 w-24 bg-[var(--bg-button)] rounded p-1" type="number" min={4} step={1} value={target} disabled={busy} onChange={(event) => setTarget(Number(event.target.value))} />
      </label>
      <p className="mt-2 text-xs text-[var(--text-muted)]">Approximate target using edge collapse. Limited to 10,000 input triangles. Shape and topology may change.</p>
    </> : <>
      ...
      <p className="mt-2 text-xs text-[var(--text-muted)]">Rebuilds a closed surface on a uniform voxel grid. Produces stepped surfaces; details smaller than one cell may disappear. Up to 100,000 input triangles.</p>
    </>}
    ...
    <button type="button" disabled={busy || !onRemesh || !valid} onClick={() => void run()} className="mt-3 px-3 py-1.5 rounded bg-[var(--accent-button)] text-white text-sm disabled:opacity-50">Apply remesh</button>
