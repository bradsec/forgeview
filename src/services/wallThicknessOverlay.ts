...
export interface WallThicknessOverlayResult {
  ...
  unsampledFaces: number
}

export function buildWallThicknessOverlay(
  meshes: THREE.Mesh[],
  minWallMm: number,
  unitInMm: number,
  isEligible: (m: THREE.Mesh) => boolean,
  highlightColor = 0xff3b30,
): WallThicknessOverlayResult {
  const group = new THREE.Group()
  group.userData.wallThicknessOverlay = true
  const toHide: THREE.Mesh[] = []
  let meshCount = 0
  let skippedMeshes = 0
  let unsampledFaces = 0
  ...
  for (const mesh of meshes) {
    ...
    const { mask, unsampledCount } = computeWallThicknessMask(worldGeo, minWallMm, unitInMm)
  ...
  return { group, hiddenMeshes: toHide, meshCount, skippedMeshes, unsampledFaces }
}

export function disposeWallThicknessOverlay(group: THREE.Group): void {
