...
import { prepareExportSave, saveExportedFile } from './saveFile'
...
describe('saveExportedFile', () => {
  ...
  it('writes to the prepared handle without opening another picker', async () => {
    const write = vi.fn().mockResolvedValue(undefined)
    const close = vi.fn().mockResolvedValue(undefined)
    const createWritable = vi.fn().mockResolvedValue({ write, close })
    const picker = vi.fn().mockResolvedValue({ name: 'chosen.stl', createWritable })
    Object.defineProperty(window, 'showSaveFilePicker', { configurable: true, value: picker })

    const pendingHandle = prepareExportSave('model.stl')
    expect(picker).toHaveBeenCalledOnce()
    const handle = await pendingHandle
    expect(createWritable).not.toHaveBeenCalled()
    await expect(saveExportedFile(new Uint8Array([1]), 'model.stl', handle)).resolves.toBe('chosen.stl')
    expect(picker).toHaveBeenCalledOnce()
    expect(write).toHaveBeenCalledWith(new Uint8Array([1]))
    expect(close).toHaveBeenCalledOnce()
  })
